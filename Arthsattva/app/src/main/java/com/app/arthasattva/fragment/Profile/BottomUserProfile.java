package com.app.arthasattva.fragment.Profile;
...
import com.app.arthasattva.databinding.FragmentBottomUserProfileBinding;
...
public class BottomUserProfile extends BottomSheetDialogFragment {
    ...
    private String user_id = "", stream_id = "";
    ProfilePOJO profilePOJO;
    private SessionManager sessionManager;

    //sendNotification for guest
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            ...
            if (getArguments().containsKey(Commn.TYPE)) {
                view_type = getArguments().getString(Commn.TYPE);
            }
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_bottom_user_profile, container, false);
    ...
    private void handleClick() {
        binding.tvJoinStream.setOnClickListener(view -> {

            //  Commn.myToast(binding.getRoot().getContext(), "coming soon...");

            checkFirst();

        });
        binding.tvReportUser.setOnClickListener(view -> {

            sendReport();

        });

        binding.ivMute.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        binding.ivKick.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                kickUserDialog();
            }
    ...
    private void kickUserDialog() {
        AlertDialog.Builder ad = new AlertDialog.Builder(binding.getRoot().getContext());
        ad.setTitle("Kick");
        ad.setMessage("Do You Want To Kick this user from your broadcast ?");
        ...
        ad.setPositiveButton("Kick", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int i) {
                dialog.dismiss();
    ...
    private void KickUser() {
        ...
        String dbRef = DBConstants.Kick_Users + "/" + stream_id + "/" + user_id;
        Map map = new HashMap<>();
        map.put(DBConstants.user_id, user_id);
        map.put(DBConstants.restrict_type, DBConstants.kick_type);

        Map final_map = new HashMap();
        final_map.put(dbRef, map);
        reference.updateChildren(final_map, (error, ref) -> {

            Commn.showDebugLog("you kick :" + user_id + " from stream");
        });
    }

    private void muteUserDialog() {
        AlertDialog.Builder ad = new AlertDialog.Builder(binding.getRoot().getContext());
        ad.setTitle("Mute");
        ad.setMessage("Do You Want To Mute this user ?");
    ...
    private void muteUser() {
        if (getActivity() != null) {
            if (!getActivity().isFinishing()) {
                if (stream_id != null && user_id != null) {
                            ...
                            .addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    if (snapshot.exists()) {
                                        ...
                                        binding.ivMute.setImageDrawable(binding.getRoot().getResources().getDrawable(R.drawable.ic_baseline_mic_off_24));
                                    } else {
                                        Commn.showDebugLog("you did'not muted him..");
                                        binding.ivMute.setImageDrawable(binding.getRoot().getResources().getDrawable(R.drawable.mute));
                                        addMute();
                                    }
                                }
    ...
    private void addMute() {
        DatabaseReference reference = database.getReference();
        String dbRef = DBConstants.Mute_Users + "/" + stream_id + "/" + user_id;
        Map map = new HashMap<>();
        map.put(DBConstants.user_id, user_id);
        map.put(DBConstants.restrict_type, DBConstants.mute_type);

        Map final_map = new HashMap();
        final_map.put(dbRef, map);
        reference.updateChildren(final_map, (error, ref) -> {

            Commn.showDebugLog("you muted :" + user_id + " from comments");
        });
    ...
    private void checkFirst() {
        ...
        FirebaseDatabase.getInstance().getReference().child(DBConstants.SingleStreamInvitation)
                .child(profilePOJO.getUserId())
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (snapshot.exists()) {
                            Commn.myToast(binding.getRoot().getContext(), "You already with someone!");
                        } else {
                            askJoinStream();
                        }
                    }
    ...
    private void getMyInfo() {
        ...
        report_adapter = new ArrayAdapter<String>(binding.getRoot().getContext(),
                android.R.layout.simple_list_item_1);


        if (DBConstants.LiveShopping.equalsIgnoreCase(view_type)) {
            binding.tvJoinStream.setVisibility(View.GONE);
            ...
            if (profilePOJO.getUserId().equalsIgnoreCase(user_id)) {
                ...
                binding.tvJoinStream.setVisibility(View.VISIBLE);
                binding.llEvent.setVisibility(View.VISIBLE);
            }

        }
        Commn.showDebugLog("view_type:" + view_type);
    }

    private void iniFireabase() {
        firebaseFirestore = FirebaseFirestore.getInstance();
        database = FirebaseDatabase.getInstance();
        //apiService = Client.getClient(DBConstants.firebase_url).create(APIService.class);
    }
    ...
    private void sendNotification() {
        ...
        if (profilePOJO.getName() != null) {
            if (!profilePOJO.getName().isEmpty()) {
                ...
                notificationRequest.setMessage("<strong>" + profilePOJO.getArthsId() + "</strong>" + " followed you in "+getString(R.string.app_name));
            }
        } else {
            notificationRequest.setMessage("<strong>" + profilePOJO.getArthsId() + "</strong>" +" followed you in "+getString(R.string.app_name));
        }
    ...
    private void askJoinStream() {
        AlertDialog.Builder ad = new AlertDialog.Builder(binding.getRoot().getContext());
        ad.setTitle("Guest");
        ad.setMessage("Do You Want To send request to join your stream?");
    ...
    private void sendJoinNotification() {
        ...
        Commn.myToast(binding.getRoot().getContext(), "Request Sent...");

        DatabaseReference streamRef = FirebaseDatabase.getInstance().getReference();
        Map streamMap = new HashMap();
        streamMap.put(DBConstants.user_image, profilePOJO.getImage());
        streamMap.put(DBConstants.request_status, "false");
        streamMap.put(DBConstants.stream_url, "");
        streamMap.put(DBConstants.user_id, user_id);
        Map finalMap = new HashMap();

        finalMap.put(DBConstants.SingleStreamInvitation + "/" + profilePOJO.getUserId() + "/" + user_id, streamMap);


        streamRef.updateChildren(finalMap, (error, ref) -> {

            if (error == null) {
                Commn.showDebugLog("database_stream" + "added on firebase");
            } else {
                Commn.showDebugLog("database_stream" + error.getMessage() + "");
            }
