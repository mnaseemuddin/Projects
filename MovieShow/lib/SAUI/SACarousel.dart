...
class SACarousel extends StatefulWidget {
  ...
  final int duration;
  const SACarousel({Key? key, required this.width, required this.height,
    required this.list, this.duration=2}) : super(key: key);
  ...
  void initState() {
    _timer = Timer.periodic(Duration(seconds: widget.duration), (timer) {
      setState(() {
        ...
        _controller.animateToPage(_counter, duration: Duration(milliseconds: 1000), curve: Curves.easeIn);
      });
    });
    super.initState();
  }
